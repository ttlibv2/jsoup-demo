...
import { Element as Element } from "./Element";
...
export class FormElement extends Element {
   ...
   */
  submit( callbackfn: ( action: string, method: string, formData: Record<string, any>  ) => Document): Document {
	  ...
	  formData(): any {
		...
		for (let el of this.elements) {
		  ...
		  if ("select" === el.normalName()) {
			...
			else if (Objects.equalsIgnoreCase(type, "checkbox") ||Objects.equalsIgnoreCase(type, "radio")) {
